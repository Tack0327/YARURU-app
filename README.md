...
家族の予定・実施作業をみんなで共有するアプリです。1つの家族グループに参加したメンバーが、予定・実施作業の登録・担当者設定・期限管理・完了管理を行えます。
...
3. `0003_recurrence_and_allday.sql` … 予定の終了時刻・終日・繰り返し用の列追加
...
- メールアドレス・パスワードによるログイン・新規登録（Supabase Auth）
- 家族グループの作成・招待コードによる参加
- 予定・実施作業の登録・編集・削除・ステータス変更（未対応 / 対応中 / 完了）
- 予定は日付＋開始時間＋終了時間で管理。終日（時間未指定）にも対応
- 予定の繰り返し登録（毎日 / 毎週 / 隔週 / 月に一度、作成時に3か月分をまとめて生成。各回は個別に編集・完了・削除可能）
- キーワード検索、種別・担当者・ステータスによる絞り込み、期限日時などによる並び替え
- ホーム画面に統合されたカレンダー表示と、期限超過・今日の作業の強調表示
